...
namespace dae {
    ...
    public:
        ...
        // Get the current offset (used by the renderer).
    ...
    private:
        ...
        std::shared_ptr<GameObject> m_target; // e.g., player
        // Screen dimensions can be set from configuration or during initialization.
        int m_screenWidth{ 640 };
        int m_screenHeight{ 480 };

        // Define dead zone margins (in screen pixels).
        float m_deadZoneX = 0.25f * m_screenWidth;  // 25% of screen width
        float m_deadZoneY = 0.25f * m_screenHeight; // 25% of screen height

        // Optional: dead zone, smoothing factors, etc.
    };
