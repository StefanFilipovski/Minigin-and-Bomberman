...
void dae::Renderer::RenderTexture(const Texture2D& texture, float x, float y, float width, float height, const SDL_Rect* srcRect) const
{
	SDL_Rect destRect{ static_cast<int>(x), static_cast<int>(y), static_cast<int>(width), static_cast<int>(height) };
	SDL_RenderCopy(m_renderer, texture.GetSDLTexture(), srcRect, &destRect);
}
